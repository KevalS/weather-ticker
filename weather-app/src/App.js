...
const App = () => {
  ...
  const groupForecastsByDate = (list) => {
    const grouped = {};
    ...
    <div className="App">
      ...
      {forecastData?.list?.length > 0 && (
        <div className="forecast">
          <h2>Forecast</h2>
          <div className='forcast-groups'>
            {Object.entries(groupForecastsByDate(forecastData.list)).map(([date, items]) => (
              <div className='forcast-group' key={date}>
                <h3>{date}</h3> {/* Display the date */}
                {items.map((item, index) => (
                  <div key={index}>
                    <div>
                      {new Date(item.dt * 1000).toLocaleTimeString('en-US', { hour12: false })} - Temp: {item.main.temp}°C
                    </div>
                  </div>
                ))}
              </div>
